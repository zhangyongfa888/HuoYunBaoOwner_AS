package zhidanhyb.huozhu.View.banner;
...
public class AutoScrollViewPager extends ViewPager
{
    ...
    private OnPagerItemClickListenner onPagerItemClickListenner;
    
    // 手指按下的时间（判断是否为点击事件时用得到）
    private long touchStart;
    
    // 手指抬起的时间（判断是否为点击事件时用得到）
    private long touchEnd;
    
    // 符合点击事件的最大时间差（判断是否为点击事件时用得到）
    public static final long CLICK_TIME = 150;
    
    // 符合点击事件的最大X差值（判断是否为点击事件时用得到）
    public static final float X_CHA = 10;
    
    // 记录用户手指按下时的x值（判断是否为点击事件时用得到）
    private float x1;
    
    // 记录用户手指抬起时的x值（判断是否为点击事件时用得到）
    private float x2;
    
    public void setOnPagerItemClickListenner(OnPagerItemClickListenner onPagerItemClickListenner)
    {
        this.onPagerItemClickListenner = onPagerItemClickListenner;
    }
    ...
    public boolean onTouchEvent(MotionEvent ev)
    {
        ...
        {
            x1 = ev.getX();// 记录按下时x值
            touchStart = System.currentTimeMillis();// 记录按下时间
        }
        
        if (ev.getAction() == MotionEvent.ACTION_UP)
        {
            x2 = ev.getX();// 记录抬起时x值
            touchEnd = System.currentTimeMillis();// 记录抬起时间
            // 时间差小于150毫秒、x差值的绝对值小于10，即视为点击事件
            if (CLICK_TIME > (touchEnd - touchStart) && Math.abs(x2 - x1) < X_CHA)
            {
                onPagerItemClickListenner.onPageItemClick(getCurrentItem());
            }
        }
	...
	public void setLayoutParams(int screenwidth, int pagerHeight) {
		this.setLayoutParams(new RelativeLayout.LayoutParams(screenwidth, pagerHeight));
	}
